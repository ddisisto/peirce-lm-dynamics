...
- `regime_tag_audit.py` — transitional audit script for the regime-tag consolidation pass (planning in `notes/regime-tag-consolidation.md`). For every trajectory: derives catalog-emitted tag (inline `shape_catalog`-style), tests a first-cut SLOTTED-HARMONIC predicate (clean ladder: len ≥ 3 peaks, peaks[1..2] at integer multiples of peaks[0] within ±1 step), runs NOPERIOD-A/B classification (`noperiod_audit`-style; B-half "bimodal H" deferred), compares against `first_batch_branches.BATCH` manifest under axis-matching rules. Output informs the design of `peirce/regime.py` resolver. Delete once the resolver lands and consumers are migrated.
- `branch_compare.py` — pair-wise reader for branch trajectories (any trajectory with `len(injections) > 1`) against their parents. Per pair: divergence statistics (first-divergence, last-divergence, divergence span, Hamming over the post-branch tail), deep-window shape comparison (period, peak acf, oscillation amplitude), heuristic 3-way outcome classification {re-absorbed, wandering, basin-switch}, and the sampling-strategy axis `T_req = gap / log(9)` — the temperature at which the alt-token would have probability 0.1 under a two-token softmax over (chosen, alt). Auto-discovers branches in the store; no manifest required. Outputs sampling-axis table, outcomes table, and T_req × outcome cross-tab. Read-only.
- `plot_branches.py` — matplotlib companion to `branch_compare.py`. Per-pair three-panel figures (entropy / `logit_gap` / `gap_over_H` overlays of parent vs branch, with the branch position marked) plus grid figures tiling H-trace overlays by outcome and (when `first_batch_branches.BATCH` is importable) by regime tag. Time axis is `[max(0, branch_position - 200), len(parent))` so post-branch evolution is shown in full. No outcome-based colouring of the branch line — visual reading stays unprimed by the heuristic classifier. Writes under `data/plots/branches/`. Read-only.
...
- `smoke_store.py` — round-trip, idempotence, extension-via-prefill, short-budget shortcut, runner cache-hit, position-0 injection identity-distinction, position-N>0 injection (C2 branching path: prefix preserves parent, injection forces token, alt-field semantics at injection step, distinct trajectory_id), query.
...
The branching surface is now concrete end-to-end (`first_batch_branches.py` runs branches; `branch_compare.py` reads them back; `plot_branches.py` renders them). Forward extensions:

- **Regime-tag consolidation** (in flight; audit landed, planning in `notes/regime-tag-consolidation.md`) — `peirce/regime.py` resolver module lifts regime-tag derivation out of `shape_catalog.py`'s inline rules and `first_batch_branches.BATCH`'s manifest column into a function-from-substrate. Prerequisite for the inference extensions below.
- **Second-batch driver.** Per the `design-reqs.md` C2 protocol's second-batch rule: Gumbel-Top-k sampling-without-replacement for K plausible alt-tokens per candidate position; requires a `step_distribution(...)` helper to recover the model's full distribution at a parent step.
