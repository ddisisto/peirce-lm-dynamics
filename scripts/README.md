...
The branching surface is now concrete end-to-end (`first_batch_branches.py` runs branches; `branch_compare.py` reads them back). Forward extensions:

- **Matplotlib panels** complementing `branch_compare.py`'s stdout tables — H-trace overlay (parent vs branch on common time axis), position-resolved `logit_gap` and `gap_over_H` deltas, peak-structure side-by-side. Cheap; no GPU. Held until the stdout reading saturates or a specific question wants a visual.
