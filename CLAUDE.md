...
*Last reviewed: stamp pinned at handoff into the Move 1.5 build session — N1 first-light landed (script `8631813`, observation `b394044`); next session builds the consolidated catalog (Move 1.5 below) that absorbs N1's follow-ups + close-logit branch-point analysis, then on to N2 (GPU back in play). If HEAD has moved past the stamp commit, skim the diff before assuming the picture encoded here is current.*
...
1.5. **Consolidated read-only catalog** *(scaffold for the C2 observations to come; foundation candidate for the next cycle)*. Combines into one renderer: (a) period-detection refinement — `dominant_period` first-peak-above-threshold not argmax-in-window, promoted to `peirce/shape.py` (three consumers now warrant it: `plot_trajectories.py`, `phase_aware_chosen.py`, this catalog). (b) Counter-vs-class slot tagging within slotted (heuristic on token-sequence consecutivity — consecutive integers / alphabet letters / numerals → counter; otherwise → class; the slot-readout *operation* is unchanged, the *interpretation* differs). (c) Position-resolved `logit_gap` and `gap_over_H` readouts, partitioned by N1's slot / scaffold / no-period tags. Surfaces commitment-strength heterogeneity within SCAFFOLD-only (commit-hard vs commit-soft vs cycle-bifurcation), slot robustness (per-recurrence committing vs fp16-noise-resolved), NOPERIOD primary observable (volatility-with-commitment vs volatility-with-marginality), and a principled candidate-list for N2. The catalog is the substrate's descriptive readout under v0.2 vocabulary — comprehensive enough to underwrite N2/N3, durable enough to outlast a re-founding (per cycle-to-cycle inheritance discipline in `design-reqs.md`). Read-only over the substrate; no GPU.

2. **Alternate-path continuation.** Engine work, modest; GPU back in play. Pick perturbation candidates from 1.5's catalog (the principled criterion is lowest `gap_over_H`; highest-H is the simplest baseline). Use `Injection` + KV-prefill to extend trajectories from the alt branch. Two questions: does perturbing in the early transient redirect the eventual collapse (transient leverage)? does perturbing at a per-cycle bifurcation position break / transition / re-absorb (cyclic leverage)? Re-absorption at the perturbation period would be the strongest signature of a stable structured attractor. Argmax-of-non-chosen is the simplest branch-selection rule; the principled extension once first results are in is Gumbel-Top-k sampling-without-replacement (Kool 2019) at the perturbation position, giving K plausible alt-branches proportional to model probability rather than a single argmax pick — see lit-review.md addendum.
