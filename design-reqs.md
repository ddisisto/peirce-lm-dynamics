...
- **Predicate set.** `eos`, `budget_cap`, `window_cap`, and `basin_capture` (the runtime structural-cycle detector from `peirce/basins.py`). Terminal-event tags persist on observations as a record of where the predicate fired, not as a population label or basin classification. **`basin_capture` is held for retirement.** Its terminal-event tags survive on existing C1 observations as historical record (and the predicate's code remains in `peirce/basins.py` for reproducibility), but new C2 runs do not include it in their predicate sets — only `eos` / `budget_cap` / `window_cap` participate in observation identity going forward. A C2-appropriate replacement (entropy-floor probe, slot-completion probe, or other shape-aware terminator) is introduced if and when a C2 move warrants one; until then, no structural-cycle terminator is in the substrate's predicate set.
...
- `scripts/representative_deep.py` — read-only renderer selecting six BOS-top-K branches and printing their L_arch observations.

The smoke tests (`smoke_engine`, `smoke_kv_parity`, `smoke_store`) and the data-producing runs (`broad_shallow`, `selection_bias`, `full_depth_extension`, `v2_calibration_probe`) are described in `scripts/README.md`. Cycle-2 read-only descriptive work extends this set; Cycle-2 inference work extends `scripts/` with new runs that use `runner.observe(...)` and persist to the same store.
