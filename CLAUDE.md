...
*Last reviewed: stamp pinned at the close of the regime-vocabulary canonisation commit — `design-reqs.md` *Open: regime vocabulary* section closed. Catalog tags (`SLOTTED` / `SCAFFOLD` / `NOPERIOD` with sub-tags `SLOTTED-CLASS` / `SLOTTED-COUNTER-INT` / `SLOTTED-COUNTER-LETTER` / `SLOTTED-MIXED`) lifted as canon, with coarse-axis precedent credited inline (`SCAFFOLD-period-1` ≈ Markov-chain fixed point at per-token granularity under Zekri's `Q_f`; `PERIODIC` vs `NOPERIOD` ≈ "fixed point or short cycle" / "long pre-recurrence" or "in recurrent class" / "in transient class" at coarser granularities). Standalone precedent review at `reports/regime-vocab-precedent.md` committed at `f7cc476`; `lit-review.md` carries an addendum pointing at it; `ideas.md` appears at root with the Ivgi 2407.06071 forward-thread parked. Next session moves: NOPERIOD specimen audit (read-only inline, observation entry), then N2 design pass (alternate-path continuation, GPU back in play; perturbation candidates from the catalog's principled list). If HEAD has moved past the stamp commit, skim the diff before assuming the picture encoded here is current.*
