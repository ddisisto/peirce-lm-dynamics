...
## Addendum (2026-05-10) — regime-vocabulary canonisation, precedent pass

A focused precedent review against Geng 2603.11228 and Zekri 2410.02724 (the two priority Markov-chain framings) was conducted to inform the regime-vocabulary canonisation in `design-reqs.md`. Standalone report at [`reports/regime-vocab-precedent.md`](reports/regime-vocab-precedent.md) (committed at `f7cc476`, read pinned to `4309b3e`); extracts terminology, formalisation scope, and within-cycle treatment from both papers section-by-section.

The original survey's *Terminology implications* section (above) anticipated this work and proposed — as a soft recommendation — names like *single-mode cycle* / *single-mode template cycle* / *class-enumeration template cycle* that reference the within-cycle distinction property directly. The mechanical re-derivation in the N1 / N1.5 catalog work produced its own tag set from the substrate's shape signals — `SLOTTED` / `SCAFFOLD` / `NOPERIOD` with sub-types — and the canonisation lifted those tags as canon rather than coining the alternative names. Rationale: the catalog tags are descriptive of the operational distinction without coining genre-labels, and crediting Markov-chain precedent for the coarse axis (`SCAFFOLD-period-1` ≈ Markov-chain fixed point at per-token granularity; `PERIODIC` vs `NOPERIOD` ≈ recurrent vs transient class) lands in `design-reqs.md` text alongside the names rather than via alternative coinage.

The within-cycle slot/scaffolding axis is confirmed un-named in the surveyed literature at section-quote level; the original survey's headline claim is preserved.

**Forward thread parked in `ideas.md`.** Ivgi et al. 2407.06071 ("From loops to oops"), cited by Zekri as the in-practice reference for repetition behaviour, may be a finer-grained precedent than either priority target. Pre-cleared by hand as overlapping at coarser granularity than the slot/scaffolding decomposition; not in scope of this canonisation. Holtzman 2019 similarly re-checked by hand and confirmed conceptually overlapping at coarser granularity; no terminology pressure from either.

---

*Verification status. Papers fetched directly: Holtzman 2019 (search-result level only, abstract verified), Wu 2502.15208 (HTML fetched, setup and metrics verified), Geng 2603.11228 (HTML fetched, framing and greedy-decoding claim verified, full PDF subsequently read in the addendum's precedent pass), Xu 2206.02369 / DITTO (search-result fetched, self-reinforcement claim verified), Marongiu 2504.01100 (HTML fetched, two-mechanism claim verified). The addendum's precedent pass also obtained full-PDF reads of Geng 2603.11228 and Zekri 2410.02724 with section-quote-level extraction. Papers cited from search-result snippets only: Welleck 2020, Su 2022, Meister 2023, Basu 2021, Chuang 2023, Li 2310.10226, Finlayson 2310.01693, Yan 2504.14218, Olsson 2022, Singh 2404.07129, Tigges 2407.10827. The latter set should be considered "verified by snippet" rather than "verified by full read"; specific quotation-level claims would want a fuller fetch before citation.*
