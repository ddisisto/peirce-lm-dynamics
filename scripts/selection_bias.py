...
with no budget cap so trajectories run to the architectural context length
L_arch=2048 (or earlier candidate-basin / EOS) under hard-cap T=0.
Adjudicates the open question raised by the v0.2 broad-shallow catalog: do
the budget_cap trajectories at depth 64 reach basins by L_arch, or remain
in transient?

Persisted via the runner. The 100 trajectories generated by broad_shallow
are reused — trajectory rows are cache-hit, then the engine prefills from
their existing 64 steps and continues stepping under selection_bias's
predicates (no budget_cap, wider basin probe). Captured candidate-basins
during broad_shallow that are still candidate-basins under selection_bias's
predicates terminate within their existing prefix with no inference; the
~71 budget_cap trajectories from broad_shallow get extended.

Reports per-trajectory: terminal event, length, captured basin signature
when captured. Aggregates: capture rate, capture-depth distribution,
basin coalescence including any new basins not in v0.2 catalog.

NUM_BRANCHES can be overridden via SELECTION_BIAS_TOP_K for smoke runs.
...
NUM_BRANCHES = int(os.environ.get("SELECTION_BIAS_TOP_K", "100"))
MAX_CYCLE_PERIOD = 32
CYCLE_WINDOW = 256
STATS_WINDOW = 256
...
def main() -> None:
    ...
    results: list[tuple[int, float, Observation, BasinSignature | None, float]] = []
    n_cached = 0
    ...
    for i, (bid, bprob) in enumerate(zip(branch_ids, branch_probs, strict=True)):
        ...
        if obs.terminal_event == "candidate-basin":
            ...
            sig = None
        results.append((bid, bprob, obs, sig, elapsed))
        print(
            f"  ... [{i:3d}] {obs.terminal_event:15s} len={obs.length:5d}  "
            f"({elapsed:6.2f}s)",
            flush=True,
        )
    ...
    terminals = Counter(o.terminal_event for _, _, o, _, _ in results)
    print("\nTerminal event distribution:")
    for tag, n in terminals.most_common():
        print(f"  {tag}: {n}")

    captured = [(i, sig) for i, (_, _, _, sig, _) in enumerate(results) if sig is not None]
    ...
    print("=" * 120)
    for i, (bid, bprob, obs, sig, _) in enumerate(results):
        ...
        print(
            ...
            f"{obs.terminal_event:15s} len={obs.length:5d} {cycle_tag} {stats_tag}  "
            f"{preview(tail_text)}"
        )
    ...
    for i, (_, _, _, sig, _) in enumerate(results):
        if sig is None:
            continue
    ...
    if terminals.get("window_cap"):
        ...
        print("=" * 120)
        for i, (bid, bprob, obs, _, _) in enumerate(results):
            if obs.terminal_event != "window_cap":
                continue
            branch = tokenizer.decode([bid])
            token_ids = [s.token_id for s in obs.steps]
            tail = token_ids[-128:]
            print(
                f"  [{i:3d}] {branch!r:>14} p={bprob:.4f} "
                f"len={obs.length:5d}  tail: {preview(tokenizer.decode(tail))}"
            )
