...
- `regime.py` — structured regime-tag resolver over per-step records. `regime_tag(steps)` returns a `RegimeTag` dataclass with three orthogonal axes: `umbrella` (SLOTTED / SCAFFOLD / NOPERIOD), `slot_character` (CLASS / COUNTER-INT / COUNTER-LETTER / MIXED, when SLOTTED), `peak_structure` (HARMONIC or None, orthogonal to slot/scaffold), plus `noperiod_cluster` (A / B, when NOPERIOD). Pure function of substrate primitives via `shape.py`; no engine, no store, no model. The `.flat` property emits a flat string for display consumers. Module constants: `SLOT_H_EPS`, `COUNTER_MATCH_THRESHOLD`, `HARMONIC_MIN_PEAKS`, `HARMONIC_LAG_TOLERANCE`, `NOPERIOD_B_RELAXED_PEAK_MIN`. Audit-informed: the strict HARMONIC predicate (`len(peaks) >= 3`, peaks[1..k-1] at integer multiples of peaks[0] within ±1) carves a clean ladder population — looser N1 annotator readings are deliberately not collapsed under HARMONIC. Tests pinned per-specimen at `tests/test_regime.py`.
